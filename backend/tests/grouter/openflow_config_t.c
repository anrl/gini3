...
uint16_t result = openflow_config_gnet_to_of_port_num(0);
CHECK(result==1);
result = openflow_config_gnet_to_of_port_num(65534);
CHECK(result==65535)
TEST_END

TEST_BEGIN("Simple openflow_config_of_to_gnet_port_num calls")
uint16_t result = openflow_config_of_to_gnet_port_num(1);
CHECK(result==0);
result = openflow_config_of_to_gnet_port_num(65535);
CHECK(result==65534)
TEST_END
