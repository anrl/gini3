...
#        sys.stdout = sys.__stdout__
    ...
    for switch in switches:
        ...
        command = "%s %s" % (VS_PROG_BIN, switchFlags)

        # remove old socket.. to fix the switch start problem
        # TODO: look at this carefully .. for now a quick fix (Mahes Oct, 2008)
        if os.path.exists("gini_socket.ctl"):
            os.unlink("gini_socket.ctl")
        ...
        if not independent:
            ...
            line = pidIn.readline()
            tm.notify(switch.name, line.strip(), "localhost")
            pidIn.close()        
        os.chdir(oldDir)
    return True
...
def createVWR(myGINI, options):
    ...
    for wrouter in myGINI.vwr:
        ...
        os.system("./WAP%s_start.sh" % wrouter.num)
        os.system("screen -d -m -L -S VWAP_%s Wireless_Parser" % wrouter.num)
        os.chdir(oldDir)
        print "[OK]"
    return True
    ...
    for uml in myGINI.vm:
        ...
        for nwIf in uml.interfaces:
            ...
            if (socketName == "fail"):
                ...
                outLine = getVMIFOutLine(nwIf, socketName)
            if (outLine):
                command += "%s " % outLine
            else:
                print "[FAILED]"
...
#        vsconf.close()
        if os.system("%s -s gw_socket.ctl -l uswitch.log -p uswitch.pid&" % VS_PROG) == 0:
            print "[OK]" 
        else:
            print "failed"
    ...
    for wap in waps:
        ...
        if (wap.name == nwIf.target):
            ...
            if switch_sharing:
                ...
                configOut.close()
                os.system("%s -s %s -l uswitch.log -p uswitch.pid &" % (VS_PROG, newDir+"/gw_socket.ctl"))
                os.chdir(oldDir)
            #else:
                #newDir = os.environ["GINI_HOME"] + "/bin/uml_virtual_switch/VS_%d" % len(nodes)
...
def getVMIFOutLine(nwIf, socketName):
    ...
    if not os.access(bakDir, os.F_OK):
        makeDir(bakDir)
    os.system("cp %s %s" % (configFile, bakDir))
...
def destroyGINI(myGINI, options):
    ...
    #os.system("killall uswitch screen")
...
def destroyVWR(wrouters, routerDir):
    for wrouter in wrouters:
        ...
        wrouter.num = wrouter.name.split("Wireless_access_point_")[-1]        
        os.system("screen -S %s -X eval quit" % ("WAP_%s" % wrouter.num))
        os.system("screen -S %s -X eval quit" % ("VWAP_%s" % wrouter.num))
...
def checkProcAlive(procName):
    ...
    os.system("pidof -x %s > %s" % (procName, GINI_TMP_FILE))
    ...
    for proc in procs:
        os.system("ps aux | grep %s > %s" % (proc, GINI_TMP_FILE))
        inFile = open(GINI_TMP_FILE)
