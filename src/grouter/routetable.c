 ...
 * of the small topologies that are targetted here... efficiency is not
 * a major concern...
 */
...
void printRouteTable(route_entry_t route_tbl[])
{
	...
	for (i = 0; i < MAX_ROUTES; i++)
		if (route_tbl[i].is_empty != TRUE)
		{
			printf("[%d]\t%s\t%s\t%s\t\teth%d\n", i, IP2Dot(tmpbuf, route_tbl[i].network),
			       IP2Dot((tmpbuf+20), route_tbl[i].netmask), IP2Dot((tmpbuf+40), route_tbl[i].nexthop), route_tbl[i].interface);
			rcount++;
		}
	printf("-----------------------------------------------------------------\n");
