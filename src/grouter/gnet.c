...
/*
 * makeInterface: this returns NULL if an interface cannot be 
 ...
 * 			  device:  eth0, eth1 (device name and device IDs are seperated from this)
...
interface_t *GNETMakeInterface(char *vsock_name, char *device, 
			   uchar *mac_addr, uchar *nw_addr, int iface_mtu, int cforce)
{
	...
	verbose(2, "[makeInterface]:: making Interface for [%s] with MAC %s and IP %s", 
	...
	{
		verbose(1, "[makeInterface]:: device %s already defined.. ", device);
		return NULL;
	}
	else
	{
		...
		verbose(2, "[makeInterface]:: trying to connect to %s..", vsock_name);
		if ((vcon = vpl_connect(vsock_name)) == NULL)
		{
			verbose(2, "[makeInterface]:: connecting as server.. ");
			// if client mode is forced.. fail here.. cannot do much!
			if (cforce)
			{
				verbose(1, "[makeInterface]:: unable to make network connection...");
				return NULL;
			}
			...
			{
				verbose(1, "[makeInterface]:: unable to make server connection.. ");
				return NULL;
			}
		...
		verbose(2, "[makeInterface]:: VPL connection made as client ");
