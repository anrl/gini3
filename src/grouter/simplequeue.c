...
// prints the message queue at the verbosity level given by vblevel
void printSimpleQueue(simplequeue_t *msgqueue, int vblevel)
{

	printf("Printing the simple queue: %s\n", msgqueue->name);
	if (msgqueue->maxsize == 0)
		printf("Maximum queue size: unlimited \n");
	else
		printf("Maximum queue size: %d \n", msgqueue->maxsize);
	printf("Block on write: ");
	...
	// we could go into the list and print each item as well.
	// TODO: extend the print functionality into the lists
}
