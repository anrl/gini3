...
class Interfaceable(Device):
    ...
    def getInterfaceProperty(self, propName, node=None, subnet=None):
        """
        Return an interface property specified by node or subnet.
        """
        if not node and not subnet:
            return self.interfaces[0][QtCore.QString(propName)]
        interface = self.getInterface(node, subnet)
        if interface:
            return interface[QtCore.QString(propName)]
         
    def setInterfaceProperty(self, prop, value, node=None, subnet=None):
        """
        Set an interface property specified by node or subnet.
        """
        if not node and not subnet:
            self.interfaces[0][QtCore.QString(prop)] = QtCore.QString(value)
            return
        interface = self.getInterface(node, subnet)
        if interface:
