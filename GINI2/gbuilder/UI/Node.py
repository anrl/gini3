...
class Node(DropItem, Item):
    ...
    def paint(self, painter, option, widget):
        ...
        painter.drawImage(QtCore.QPoint(-self.image.width()/2, -self.image.height()/2), self.image)
        if options["names"]:
            painter.drawText(QtCore.QRectF(-70, self.image.height()/2, 145, 60), self.getProperty("Name"), QtGui.QTextOption(QtCore.Qt.AlignHCenter))
    ...
    def attach(self):
        ...
        if not isinstance(self, Interfaceable):
            return

        #if self.shell and self.shell.isRunning():
            #return
    ...
    def contextMenu(self, pos):
        ...
        from Core.Interfaceable import Interfaceable
        if isinstance(self, Interfaceable):
        ...
        #if self.type == "Switch":
            #self.menu.addAction("Reset", self.restart)
        self.menu.exec_(pos)
    ...
    def wireshark(self):
        ...
        """
        outfile = environ["tmp"].replace("\\", "/") + str(self.getName()) + ".out"
    ...
    def graph(self):
        """
        Graph the router stats of the running device.
        """
        if options["graphing"]:
            from GraphWindow import GraphWindow
        self.rstatsWindow = GraphWindow(self.getName(), mainWidgets["canvas"])
